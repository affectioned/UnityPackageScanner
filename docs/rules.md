...
Detects managed assemblies whose metadata shows signs of obfuscation: control characters in type or method names, a high ratio of single-character identifiers, known obfuscator marker attributes such as [Obfuscation] or [ConfusedBy], or string literals whose character-distribution entropy indicates encrypted or randomly-generated content. Obfuscation alone is not malicious — commercial packages sometimes protect their IP — but obfuscated code from an unknown source is harder to audit and warrants extra scrutiny, particularly when combined with network-access or process-spawn findings.
