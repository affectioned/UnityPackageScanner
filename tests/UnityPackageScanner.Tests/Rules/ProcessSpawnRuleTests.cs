...
public sealed class ProcessSpawnRuleTests
{
    private readonly ProcessSpawnRule _rule = new(NullLogger<ProcessSpawnRule>.Instance);
    private readonly UnityPackageExtractor _extractor = new(NullLogger<UnityPackageExtractor>.Instance);

    // --- Positive tests ---

    [Fact]
    public async Task Fires_on_Process_call()
    {
        var entries = await BuildAndExtract("Assets/Plugins/evil.dll", ManagedDllBuilder.WithProcessSpawn());
        var findings = await CollectFindings(entries);

        findings.Should().ContainSingle()
            .Which.RuleId.Should().Be(KnownRuleIds.ProcessSpawn);
    }
    ...
    [Fact]
    public async Task Evidence_mentions_Process()
    {
        var entries = await BuildAndExtract("Assets/Plugins/evil.dll", ManagedDllBuilder.WithProcessSpawn());
        var findings = await CollectFindings(entries);

        findings.Single().Evidence.Should().Contain("Process");
    }
